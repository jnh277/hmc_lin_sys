...
transformed parameters{
    ...
    for (i in max_order+1:no_obs_est){
        mu[i] = y_est[i]+y_est[i-output_order:i-1]*f_coefs - u_est[i-input_order+1:i] * b_coefs
                -ehat[i-output_order:i-1]*f_coefs;
    }
    r2 = sqrt(sum(f_coefs .* f_coefs * r*r));
...
//    ehat ~ normal(0.0, r);
    ...
    ehat[1:max_order] ~ normal(0.0, r);     // this includes the e_init prior
    ehat[max_order+1:no_obs_est] ~ normal(mu[max_order+1:no_obs_est], r2);
...
generated quantities {
    ...
    for (i in max_order+1:no_obs_val){ // this isn't the best estimate of y_val as it doesnt have the error terms?
        y_hat_val[i] = u_val[i-input_order+1:i] * b_coefs
                - y_val[i-output_order:i-1] * f_coefs;//
